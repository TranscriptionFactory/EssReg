#' @export
cleanData <- function(xdata, ydata, mode = 1,
  ...
  if ( !is.null(xdata)) {
    ...
    if (scale_zeroes_directly > 0) {
      x_rows_convert = which(apply(xdata, 1, function(x) x))
      x_cols_convert = which(apply(xdata, 2, function(x) x))
      
      xrows = xdata[x_rows_convert, ] = 0.001
      xcols = xdata[, x_cols_convert] = 0.001
    }
    # remove zero SD too
